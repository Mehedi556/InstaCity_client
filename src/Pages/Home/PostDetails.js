...
const PostDetails = () => {
  ...
  const {
    ...
    queryKey: ['comments'],
    queryFn: () =>
      fetch(`https://insta-server-murex.vercel.app/comments/${post._id}`).then(
        res => res.json()
      ),
  });
  ...
  const setData = (comment, postId, userName) => {
    ...
    fetch('https://insta-server-murex.vercel.app/comments', {
      method: 'POST',
      headers: {
      ...
      <div className="card w-12/12 md:w-9/12 bg-base-100 shadow-xl mx-auto rounded-none border p-4 rounded-lg">
        ...
        <div className="card-body">
          <p className="font-bold">
            Post created by:{' '}
            <span className="text-xl">{post?.displayName}</span>
          </p>
          <p className="font-bold">
            Creators email: <span className="text-xl">{post?.email}</span>
          </p>
        </div>
        <form onSubmit={handleSubmit(handlePost)} className="w-11/12 mx-auto">
          <div className="form-control">
            <input
              ...
              name="comment"
              id=""
              className="input input-sm input-bordered w-11/12 mx-auto"
            />
          </div>
        ...
        {commentData && (
          <div>
            <h1 className="mt-10 font-bold text-xl p-2">
